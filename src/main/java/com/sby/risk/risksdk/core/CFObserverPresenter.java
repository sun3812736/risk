...
import com.sby.risk.risksdk.presenter.CFDeblockObserver;
import com.sby.risk.risksdk.presenter.CFNetworkObserver;
import com.sby.risk.risksdk.presenter.CFScreenObserver;
 ...
 */
public class CFObserverPresenter implements CFNetworkObserver.INetworkListener, CFScreenObserver.IScreenListener,
        CFDeblockObserver.IKeyguardListener {

    /** NetworkObserver  */
    private CFNetworkObserver mNetworkObserver;
    /** ScreenObserver */
    private CFScreenObserver mScreenObserver;
    /** DeblockObserver */
    private CFDeblockObserver mKeyguardObserver;
    /** isForeground */
    ...
    /** TAG */
    private static final String LOG_TAG = "SBYStat::ObserverPresenter";

    private ScheduleListener scheduleListener;


    public CFObserverPresenter(ScheduleListener listener) {
    ...
    private void registerObserver(Context aContext) {
        ...
        if (mScreenObserver == null) {
            mScreenObserver = new CFScreenObserver(aContext, this);
        }
        mScreenObserver.start();

        if (mNetworkObserver == null) {
            mNetworkObserver = new CFNetworkObserver(aContext, this);
        }
        mNetworkObserver.start();

        if (mKeyguardObserver == null) {
            mKeyguardObserver = new CFDeblockObserver(aContext, this);
        }
...
//        reportData(aContext);
        ...
        if (aIsForeground) {
            ...
            //切后台时数据上传
            CFStatSdk.getInstance(aContext).send();
            // 切后台停止计时
            scheduleStop();
    ...
    public void onNetworkConnected(Context aContext) {
        ...
        // 同步网络信息
        HeadrHandle.getHeader(aContext).setNetworkinfo(HeadrHandle.getNetWorkInfo(aContext));
        if (isForeground) {
